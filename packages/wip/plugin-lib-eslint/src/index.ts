import plugin, { HarkDataFilesProps, HarkDataFile } from "@hark/plugin";
import { CLIEngine } from "eslint";

export default (userOptions?: CLIEngine.Options, formatter = "") =>
  plugin("eslint", ({ logMessage, logPath }) => async ({ files }: HarkDataFilesProps) => {
    ...
    const report: CLIEngine.LintReport = filesToCheck.reduce(
      (acc, file) => {
